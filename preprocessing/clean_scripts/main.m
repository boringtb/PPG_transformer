...
testOutDir = '/collab2/ktang5/cleaned/';
dirName = '/collab2/ktang5/out_test/';   % Location of the data folder
...
%filter
f_type = 'Butterworth';    %Needs to be further implemented
...
for i = 3:numel(dirData)
    ...
    %for j = 1:1  % for testing purposes
        ...
        norm_data(1,:) = normalize_signal(new_data(1,:));
        norm_data(2,:) = new_data(2,:);
        
        % make a feature matrix and a matrix of raw values
        [feat, raw] = make_matrices(norm_data, fsppg, nA2, nB2, fullname, false, true, testOutDir); % deliniator
